...
var Transaction = siriuscore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/sirius.conf')));
var siriusService = proxyquire('../../lib/services/siriusd', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultsiriusConf = fs.readFileSync(path.resolve(__dirname, '../data/default.sirius.conf'), 'utf8');

describe('sirius Service', function() {
	...
	var baseConfig = {
		node: {
			network: siriuscore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var siriusd = new siriusService(baseConfig);
			should.exist(siriusd);
		});
		it('will create an instance without `new`', function() {
			var siriusd = siriusService(baseConfig);
			should.exist(siriusd);
		});
		it('will init caches', function() {
			var siriusd = new siriusService(baseConfig);

			should.exist(siriusd.utxosCache);
			should.exist(siriusd.txidsCache);
			should.exist(siriusd.balanceCache);
			should.exist(siriusd.summaryCache);
			should.exist(siriusd.transactionDetailedCache);
			should.exist(siriusd.accountInfo);

			should.exist(siriusd.transactionCache);
			should.exist(siriusd.rawTransactionCache);
			should.exist(siriusd.rawJsonTransactionCache);
			should.exist(siriusd.blockCache);
			should.exist(siriusd.blockJsonCache);
			should.exist(siriusd.blockSubsidyCache);
			should.exist(siriusd.rawBlockCache);
			should.exist(siriusd.blockHeaderCache);
			should.exist(siriusd.zmqKnownTransactions);
			should.exist(siriusd.zmqKnownBlocks);
			should.exist(siriusd.lastTip);
			should.exist(siriusd.lastTipTimeout);
			should.equal(siriusd.dgpInfoCache, null, 'should be null');
			should.equal(siriusd.miningInfoCache, null, 'should be null');
			should.equal(siriusd.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(siriusd.maxTxids, 1000);
			should.equal(siriusd.maxTransactionHistory, 50);
			should.equal(siriusd.maxAddressesQuery, 10000);
			should.equal(siriusd.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(siriusd.spawnRestartTime, 5000);
			should.equal(siriusd.spawnStopTime, 10000);

			// try all interval
			should.equal(siriusd.tryAllInterval, 1000);
			should.equal(siriusd.startRetryInterval, 5000);

			// rpc limits
			should.equal(siriusd.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(siriusd.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var siriusd = new siriusService(baseConfig);
			siriusd.nodes.should.deep.equal([]);
			siriusd.nodesIndex.should.equal(0);
			siriusd.nodes.push({ client: sinon.stub() });
			should.exist(siriusd.client);
		});
		it('will set subscriptions', function() {
			var siriusd = new siriusService(baseConfig);
			siriusd.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var siriusd = new siriusService(baseConfig);
			siriusd._initDefaults({ transactionConcurrency: 10 });
			siriusd.transactionConcurrency.should.equal(10);
			siriusd._initDefaults({});
			siriusd.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			siriusService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var siriusd = new siriusService(baseConfig);
			var methods = siriusd.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var siriusd = new siriusService(baseConfig);
			var events = siriusd.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('siriusd/rawtransaction');
			events[0].scope.should.equal(siriusd);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('siriusd/hashblock');
			events[1].scope.should.equal(siriusd);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('siriusd/addresstxid');
			events[2].scope.should.equal(siriusd);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('siriusd/addressbalance');
			events[3].scope.should.equal(siriusd);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var siriusd = new siriusService(baseConfig);
			siriusd.subscribe = sinon.stub();
			siriusd.unsubscribe = sinon.stub();
			var events = siriusd.getPublishEvents();

			events[0].subscribe('test');
			siriusd.subscribe.args[0][0].should.equal('rawtransaction');
			siriusd.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			siriusd.unsubscribe.args[0][0].should.equal('rawtransaction');
			siriusd.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			siriusd.subscribe.args[1][0].should.equal('hashblock');
			siriusd.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			siriusd.unsubscribe.args[1][0].should.equal('hashblock');
			siriusd.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var siriusd = new siriusService(baseConfig);
			var emitter = {};
			siriusd.subscribe('hashblock', emitter);
			siriusd.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			siriusd.subscribe('rawtransaction', emitter2);
			siriusd.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			siriusd.subscribe('hashblock', emitter1);
			siriusd.subscribe('hashblock', emitter2);
			siriusd.subscribe('hashblock', emitter3);
			siriusd.subscribe('hashblock', emitter4);
			siriusd.subscribe('hashblock', emitter5);
			siriusd.subscriptions.hashblock.length.should.equal(5);

			siriusd.unsubscribe('hashblock', emitter3);
			siriusd.subscriptions.hashblock.length.should.equal(4);
			siriusd.subscriptions.hashblock[0].should.equal(emitter1);
			siriusd.subscriptions.hashblock[1].should.equal(emitter2);
			siriusd.subscriptions.hashblock[2].should.equal(emitter4);
			siriusd.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var siriusd = new siriusService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			siriusd.subscriptions.hashblock = [emitter1];
			siriusd.unsubscribe('hashblock', emitter3);
			siriusd.subscriptions.hashblock.length.should.equal(1);
			siriusd.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			siriusd.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(siriusd.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var siriusd = new siriusService(baseConfig);
			var emitter = new EventEmitter();
			siriusd.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var siriusd = new siriusService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			siriusd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			siriusd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			siriusd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			siriusd.unsubscribeAddress(emitter1);
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			siriusd.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			siriusd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			siriusd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			siriusd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(siriusd, 'unsubscribeAddressAll');
			siriusd.unsubscribeAddress(emitter1);
			siriusd.unsubscribeAddressAll.callCount.should.equal(1);
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			siriusd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			siriusd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			siriusd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			siriusd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			siriusd.unsubscribeAddressAll(emitter1);
			siriusd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			siriusd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			siriusd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(siriusd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var siriusd = new siriusService(baseConfig);
			var config = siriusd._getDefaultConfig();
			config.should.equal(defaultsiriusConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a sirius.conf file', function() {
			var Testsirius = proxyquire('../../lib/services/siriusd', {
				fs: {
					readFileSync: readFileSync,
			...
			var siriusd = new Testsirius(baseConfig);
			siriusd.options.spawn.datadir = '/tmp/.sirius';

			sinon.spy(siriusd, '_expandRelativeDatadir');
			sinon.spy(siriusd, '_checkConfigIndexes');
			sinon.spy(siriusd, '_getDefaultConf')
			sinon.spy(siriusd, '_getNetworkConfigPath');
			sinon.spy(siriusd, '_parseBitcoinConf');

			var node = {};
			siriusd._loadSpawnConfiguration(node);

			siriusd._expandRelativeDatadir.callCount.should.equal(1);
			siriusd._checkConfigIndexes.callCount.should.equal(1);
			siriusd._getDefaultConf.callCount.should.equal(1);
			siriusd._getNetworkConfigPath.callCount.should.equal(1);
			siriusd._parseBitcoinConf.callCount.should.equal(2);

			should.exist(siriusd.spawn.config);
			siriusd.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: siriuscore.Networks.testnet,
					configPath: '/tmp/.siriuscore/siriuscore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			var siriusd = new Testsirius(config);
			...
			siriusd.options.spawn.datadir = './data';
			var node = {};
			siriusd._loadSpawnConfiguration(node);
			...
			siriusd.options.spawn.datadir.should.equal('/tmp/.siriuscore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var Testsirius = proxyquire('../../lib/services/siriusd', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badsirius.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var siriusd = new Testsirius(baseConfig);
			(function() {
				siriusd._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(siriuscore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the sirius.conf file', function() {
			var siriusd = new siriusService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				siriusd._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the sirius.conf file', function() {
			var siriusd = new siriusService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the sirius.conf file', function() {
			var siriusd = new siriusService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the sirius.conf file', function() {
			var siriusd = new siriusService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the sirius.conf file', function() {
			var siriusd = new siriusService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				siriusd.transactionDetailedCache.set(keys[i], {});
				siriusd.utxosCache.set(keys[i], {});
				siriusd.accountInfo.set(keys[i], {});
				siriusd.txidsCache.set(keys[i], {});
				siriusd.balanceCache.set(keys[i], {});
				siriusd.summaryCache.set(keys[i], {});
				siriusd.blockOverviewCache.set(keys[i], {});
				siriusd.blockJsonCache.set(keys[i], {});
			}
			siriusd._resetCaches();
			should.equal(siriusd.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(siriusd.utxosCache.get(keys[0]), undefined);
			should.equal(siriusd.accountInfo.get(keys[0]), undefined);
			should.equal(siriusd.txidsCache.get(keys[0]), undefined);
			should.equal(siriusd.balanceCache.get(keys[0]), undefined);
			should.equal(siriusd.summaryCache.get(keys[0]), undefined);
			should.equal(siriusd.blockOverviewCache.get(keys[0]), undefined);
			should.equal(siriusd.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			siriusd._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				siriusd.nodes[0].client.getInfo.callCount.should.equal(1);
				siriusd.nodes[1].client.getInfo.callCount.should.equal(1);
				siriusd.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			siriusd._tryAllClients(function(client, next) {
				...
				siriusd.nodes[1].client.getInfo.callCount.should.equal(1);
				siriusd.nodes[2].client.getInfo.callCount.should.equal(1);
				siriusd.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert siriusd-rpc error object into JavaScript error', function() {
			var siriusd = new siriusService(baseConfig);
			var error = siriusd._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			siriusd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			siriusd.nodes.push({
				client: {
			...
			siriusd._initChain(function() {
				log.info.callCount.should.equal(1);
				siriusd.getRawBlock.callCount.should.equal(1);
				siriusd.getRawBlock.args[0][0].should.equal('genesishash');
				siriusd.height.should.equal(5000);
				siriusd.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		it('it will handle error from getBestBlockHash', function(done) {
			...
			siriusd._initChain(function(err) {
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			siriusd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			siriusd._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			...
			baseConfig.node.network = siriuscore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: siriuscore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var siriusd = new siriusService(config);
			siriusd._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var siriusd = new siriusService(config);
			siriusd._getDefaultConf().rpcport.should.equal(18332);
		});
		it('will get default rpc port for regtest', function() {
			siriuscore.Networks.enableRegtest();
			var config = {
				node: {
					network: siriuscore.Networks.testnet
				},
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var siriusd = new siriusService(config);
			should.equal(siriusd._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var siriusd = new siriusService(config);
			siriusd._getNetworkConfigPath().should.equal('testnet3/sirius.conf');
		});
		it('will get default rpc port for regtest', function() {
			siriuscore.Networks.enableRegtest();
			var config = {
			...
			var siriusd = new siriusService(config);
			siriusd._getNetworkConfigPath().should.equal('regtest/sirius.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var siriusd = new siriusService(baseConfig);
			siriusd.node.network = siriuscore.Networks.testnet;
			siriusd._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var siriusd = new siriusService(baseConfig);
			siriusd.node.network = siriuscore.Networks.testnet;
			siriuscore.Networks.enableRegtest();
			siriusd._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var siriusd = new siriusService(baseConfig);
			siriusd.node.network = siriuscore.Networks.livenet;
			siriuscore.Networks.enableRegtest();
			should.equal(siriusd._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			siriusd._rapidProtectedUpdateTip = sinon.stub();
			siriusd.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			siriusd._zmqBlockHandler(node, message);
			siriusd._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			siriusd._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(siriusd, 'emit');
			siriusd.on('block', function(block) {
				block.should.equal(message);
				siriusd.emit.callCount.should.equal(1);
				siriusd._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			siriusd._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			siriusd._zmqBlockHandler(node, message);
			siriusd._rapidProtectedUpdateTip.callCount.should.equal(1);
			siriusd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			siriusd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(siriusd, 'emit');
			var emitter = new EventEmitter();
			siriusd.subscriptions.hashblock.push(emitter);
			emitter.on('siriusd/hashblock', function(blockHash) {
				siriusd._rapidProtectedUpdateTip.callCount.should.equal(1);
				siriusd.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				siriusd._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.syncPercentage = sinon.stub();
			sinon.spy(siriusd, '_resetCaches');
			siriusd.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				siriusd._resetCaches.callCount.should.equal(1);
			...
			siriusd._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			siriusd.on('synced', function() {
				siriusd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			siriusd.on('synced', function() {
				siriusd.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			siriusd.on('error', function(err) {
				log.error.callCount.should.equal(1);
				siriusd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.syncPercentage = sinon.stub();
			siriusd._resetCaches = sinon.stub();
			siriusd.on('tip', function(height) {
				siriusd._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				siriusd.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using siriuscore.Transaction', function() {
			var siriusd = new siriusService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = siriuscore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(siriuscore.Networks.testnet);
			var outputAddress = siriuscore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = siriuscore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: siriuscore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = siriusd._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addOutput(siriuscore.Transaction.Output({
				script: siriuscore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = siriusd._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var siriusd = new siriusService(baseConfig);
			var tx = siriuscore.Transaction();
			var address = siriuscore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(siriuscore.Transaction.Output({
				script: siriuscore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(siriuscore.Transaction.Output({
				script: siriuscore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			siriusd._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			siriusd.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('siriusd/addresstxid', function(data) {
			...
			siriusd._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			siriusd.subscriptions.rawtransaction.push(emitter);
			emitter.on('siriusd/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			siriusd._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(siriusd, 'emit');
			siriusd.subscriptions.rawtransaction.push(emitter);
			emitter.on('siriusd/rawtransaction', function() {
				done();
			});
			var node = {};
			siriusd._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd._updateTip = sinon.stub();
			siriusd._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			siriusd.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			siriusd._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				siriusd._updateTip.callCount.should.equal(11);
				siriusd._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			siriusd._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				siriusd._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			siriusd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				siriusd._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			siriusd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				siriusd._zmqBlockHandler.callCount.should.equal(0);
				siriusd._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var siriusd = new siriusService(baseConfig);
			var node = {};
			siriusd._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			siriusd._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			siriusd._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			siriusd._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
		...
		it('will set height and emit tip', function(done) {
			...
			siriusd.on('tip', function(height) {
				...
				siriusd.height.should.equal(100);
				done();
			});
			siriusd._loadTipFromNode(node, function(err) {
				if (err) {
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestsiriusService = proxyquire('../../lib/services/siriusd', {
				fs: {
					readFile: readFile
				}
			});
			var siriusd = new TestsiriusService(baseConfig);
			siriusd.spawnStopTime = 1;
			siriusd._process = {};
			siriusd._process.kill = sinon.stub();
			siriusd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				siriusd._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			siriusd._process.kill = sinon.stub().throws(error2);
			siriusd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd._loadSpawnConfiguration = sinon.stub();
			siriusd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			siriusd._spawnChildProcess(function(err) {
				siriusd._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var siriusd = new siriusService(baseConfig);
			siriusd._loadSpawnConfiguration = sinon.stub();
			siriusd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			siriusd._spawnChildProcess(function(err) {
				siriusd._loadSpawnConfiguration.callCount.should.equal(1);
				siriusd._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var siriusd = new TestsiriusService(config);
			siriusd.spawn = {};
			siriusd._loadSpawnConfiguration = sinon.stub();
			siriusd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			siriusd.node.stopping = true;
			siriusd._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			siriusd._loadSpawnConfiguration = sinon.stub();
			siriusd.spawn = {};
			siriusd.spawn.exec = 'testexec';
			siriusd.spawn.configPath = 'testdir/sirius.conf';
			siriusd.spawn.datadir = 'testdir';
			siriusd.spawn.config = {};
			siriusd.spawn.config.rpcport = 20001;
			siriusd.spawn.config.rpcuser = 'sirius';
			siriusd.spawn.config.rpcpassword = 'password';
			siriusd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			siriusd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			siriusd._initZmqSubSocket = sinon.stub();
			siriusd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			siriusd._checkReindex = sinon.stub().callsArgWith(1, null);
			siriusd._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/sirius.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				siriusd._loadTipFromNode.callCount.should.equal(1);
				siriusd._initZmqSubSocket.callCount.should.equal(1);
				should.exist(siriusd._initZmqSubSocket.args[0][0].client);
				siriusd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				siriusd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(siriusd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn siriusd spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestsiriusService = proxyquire('../../lib/services/siriusd', {
				fs: {
			...
			siriusd._loadSpawnConfiguration = sinon.stub();
			siriusd.spawn = {};
			siriusd.spawn.exec = 'siriusd';
			siriusd.spawn.datadir = '/tmp/sirius';
			siriusd.spawn.configPath = '/tmp/sirius/sirius.conf';
			siriusd.spawn.config = {};
			siriusd.spawnRestartTime = 1;
			siriusd._loadTipFromNode = sinon.stub().callsArg(1);
			siriusd._initZmqSubSocket = sinon.stub();
			siriusd._checkReindex = sinon.stub().callsArg(1);
			siriusd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			siriusd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(siriusd, '_spawnChildProcess');
			siriusd._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						siriusd._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			siriusd._spawnChildProcess(function(err) {
				...
				siriusd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				siriusd.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn siriusd spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestsiriusService = proxyquire('../../lib/services/siriusd', {
				fs: {
			...
			siriusd._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						siriusd._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			siriusd.spawn.config.rpcpassword = 'password';
			siriusd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			siriusd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			siriusd._spawnChildProcess(function(err) {
				siriusd._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			siriusd._initZmqSubSocket = sinon.stub();
			siriusd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			siriusd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var siriusd = new siriusService(config);
			siriusd.node.stopping = true;
			siriusd.startRetryInterval = 100;
			siriusd._loadTipFromNode = sinon.stub();
			siriusd._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to siriusd/);
				siriusd._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			siriusd.startRetryInterval = 1;
			var config = {};
			siriusd._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				siriusd._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			siriusd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			siriusd._connectProcess(config, function(err, node) {
				should.not.exist(err);
				siriusd._loadTipFromNode.callCount.should.equal(1);
				siriusd._initZmqSubSocket.callCount.should.equal(1);
				siriusd._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.options = {};
			siriusd.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('sirius configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			siriusd.options = {
				connect: [
					{}
				]
			...
			siriusd.start(function(err) {
				siriusd._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var siriusd = new siriusService(baseConfig);
			var node = {};
			siriusd._initChain = sinon.stub().callsArg(0);
			siriusd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			siriusd.options = {
				spawn: {}
			};
			siriusd.start(function(err) {
				should.not.exist(err);
				siriusd.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(siriusd, '_spawnChildProcess');
			var nodes = [{}];
			siriusd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			siriusd.options = {
				connect: [
					{}
				]
			...
			siriusd.start(function(err) {
				should.not.exist(err);
				siriusd._initChain.callCount.should.equal(1);
				siriusd._connectProcess.callCount.should.equal(1);
				siriusd.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			siriusd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				siriusd.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			siriusd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			siriusd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			siriusd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.syncPercentage(function(err) {
				should.exist(err);
				siriusd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			siriusd.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				siriusd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var siriusd = new siriusService(baseConfig);
			var args = siriusd._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var siriusd = new siriusService(baseConfig);
			var args = siriusd._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var siriusd = new siriusService(baseConfig);
			sinon.spy(siriusd, '_normalizeAddressArg');
			siriusd.nodes.push({
				client: {
			...
			var options = {};
			siriusd.getAddressBalance(address, options, function(err) {
				siriusd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			siriusd.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				siriusd.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					siriusd._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			siriusd.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = siriusd._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = siriusd._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = siriusd._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = siriusd._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				siriusd._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			siriusd.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(siriusd, '_getHeightRangeQuery');
			sinon.spy(siriusd, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			siriusd.getAddressTxids(address, options, function(err) {
				should.exist(err);
				siriusd._getHeightRangeQuery.callCount.should.equal(1);
				siriusd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			siriusd.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				siriusd._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			siriusd.getAddressTxids(address, options, function(err, txids) {
				...
				siriusd.getAddressTxids(address, options, function(err, txids) {
					...
					siriusd._getHeightRangeQuery.callCount.should.equal(2);
					siriusd._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			siriusd.getAddressTxids(address, options, function(err, txids) {
				...
				siriusd.getAddressTxids(address, options, function(err, txids) {
					...
					siriusd._getHeightRangeQuery.callCount.should.equal(4);
					siriusd._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			siriusd.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				siriusd.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					siriusd.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						siriusd._getHeightRangeQuery.callCount.should.equal(3);
						siriusd._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var siriusd = new siriusService(baseConfig);
			siriusd.height = 10;
			var confirmations = siriusd._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var siriusd = new siriusService(baseConfig);
			var result = {};
			siriusd._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var siriusd = new siriusService(baseConfig);
			var result = {};
			siriusd._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var siriusd = new siriusService(baseConfig);
			var result = {};
			siriusd._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var siriusd = new siriusService(baseConfig);
			var result = {};
			siriusd._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(siriusd, '_getAddressDetailsForInput');
			sinon.spy(siriusd, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = siriusd._getAddressDetailsForTransaction(tx, addresses);
			...
			siriusd._getAddressDetailsForInput.callCount.should.equal(1);
			siriusd._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var siriusd = new siriusService(baseConfig);
			sinon.spy(siriusd, '_getConfirmationsDetail');
			siriusd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			siriusd.height = 300;
			var addresses = {};
			siriusd._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			siriusd._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				siriusd.getDetailedTransaction.callCount.should.equal(1);
				siriusd._getAddressDetailsForTransaction.callCount.should.equal(1);
				siriusd._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var siriusd = new siriusService(baseConfig);
			siriusd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			siriusd._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				siriusd.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from siriuscore addresses', function() {
			var addresses = [
				siriuscore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				siriuscore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var siriusd = new siriusService(baseConfig);
			var strings = siriusd._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var siriusd = new siriusService(baseConfig);
			(function() {
				siriusd._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var siriusd = new siriusService(baseConfig);
			sinon.spy(siriusd, '_normalizeAddressArg');
			siriusd.maxAddressesQuery = 100;
			siriusd.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				siriusd._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var siriusd = new siriusService(baseConfig);
			sinon.spy(siriusd, '_normalizeAddressArg');
			sinon.spy(siriusd, '_getAddressStrings');
			siriusd.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				siriusd._normalizeAddressArg.callCount.should.equal(1);
				siriusd._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(siriusd, '_normalizeAddressArg');
			sinon.spy(siriusd, '_getAddressStrings');
			siriusd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			siriusd.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				siriusd._normalizeAddressArg.callCount.should.equal(1);
				siriusd._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(siriusd, '_normalizeAddressArg');
			sinon.spy(siriusd, '_getAddressStrings');
			siriusd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			siriusd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			siriusd.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				siriusd._normalizeAddressArg.callCount.should.equal(1);
				siriusd._getAddressStrings.callCount.should.equal(1);
				siriusd.getAddressTxids.callCount.should.equal(1);
				siriusd._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(siriusd, '_normalizeAddressArg');
			sinon.spy(siriusd, '_getAddressStrings');
			sinon.spy(siriusd, 'getAddressTxids');
			siriusd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			siriusd.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				siriusd._normalizeAddressArg.callCount.should.equal(1);
				siriusd._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(siriusd, '_normalizeAddressArg');
			sinon.spy(siriusd, '_getAddressStrings');
			sinon.spy(siriusd, '_paginateTxids');
			siriusd._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(siriusd, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			siriusd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			siriusd.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				siriusd._normalizeAddressArg.callCount.should.equal(1);
				siriusd._getAddressStrings.callCount.should.equal(1);
				siriusd.getAddressTxids.callCount.should.equal(1);
				siriusd._getAddressDetailedTransaction.callCount.should.equal(2);
				siriusd._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			siriusd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			siriusd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			siriusd.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			siriusd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			siriusd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			siriusd.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			siriusd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			siriusd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			siriusd.getAddressSummary(address, options, function(err, summary) {
				siriusd._paginateTxids.callCount.should.equal(1);
				siriusd._normalizeAddressArg.callCount.should.equal(1);
				siriusd.getAddressTxids.callCount.should.equal(1);
				siriusd.getAddressBalance.callCount.should.equal(1);
				siriusd._paginateTxids.args[0][1].should.equal(0);
				siriusd._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			siriusd.getAddressSummary(address, options, function(err, summary) {
				...
				siriusd.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					siriusd._paginateTxids.callCount.should.equal(0);
					siriusd._normalizeAddressArg.callCount.should.equal(2);
					siriusd.getAddressTxids.callCount.should.equal(1);
					siriusd.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			siriusd.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				siriusd._paginateTxids.callCount.should.equal(1);
				siriusd._normalizeAddressArg.callCount.should.equal(1);
				siriusd.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			siriusd._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			siriusd.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(siriusd, '_tryAllClients');
			siriusd.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			siriusd.getRawBlock(blockhash, function(err) {
				should.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			siriusd.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			siriusd.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			siriusd.getBlock(0, function(err) {
				siriusd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as siriuscore object from height', function(done) {
			var siriusd = new siriusService(baseConfig);
			sinon.spy(siriusd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			siriusd.getBlock(0, function(err, block) {
				should.not.exist(err);
				siriusd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(siriuscore.Block);
				done();
			});
		});
		it('will getblock as siriuscore object', function(done) {
			var siriusd = new siriusService(baseConfig);
			sinon.spy(siriusd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			siriusd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			siriusd.getBlock(hash, function(err, block) {
				should.not.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			siriusd.getBlock(0, function(err, block) {
				...
				siriusd.getBlock(0, function(err, block) {
					should.not.exist(err);
					siriusd._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			siriusd.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var siriusd = new siriusService(baseConfig);
			sinon.spy(siriusd, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			siriusd.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			siriusd.getBlockHeader(10, function(err) {
				siriusd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			siriusd.getBlockHeader(blockhash, function(err) {
				siriusd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			siriusd.getBlockHeader(0, function(err) {
				siriusd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			siriusd.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				siriusd._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			siriusd.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				siriusd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			siriusd._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				siriusd._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			siriusd._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			siriusd._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var siriusd = new siriusService(baseConfig);
			sinon.spy(siriusd, '_maybeGetBlockHash');
			siriusd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			siriusd.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				siriusd._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			siriusd.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				siriusd.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					siriusd._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			siriusd.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = siriuscore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			siriusd.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			siriusd.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			siriusd.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = siriuscore.Transaction();
			(function() {
				siriusd.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getRawTransaction('txid', function(err) {
				should.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			siriusd.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getTransaction('txid', function(err) {
				should.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			siriusd.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				siriusd._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(siriuscore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			siriusd.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				siriusd._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			siriusd.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			siriusd.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			siriusd.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.node.getNetworkName = sinon.stub().returns('testnet');
			...
			siriusd.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			siriusd.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			siriusd.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				siriusd.getSubsidy.callCount.should.equal(1);
				siriusd.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			siriusd.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.spawn = {};
			siriusd.spawn.process = new EventEmitter();
			siriusd.spawn.process.kill = sinon.stub();
			siriusd.stop(done);
			siriusd.spawn.process.kill.callCount.should.equal(1);
			siriusd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			siriusd.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			siriusd.spawn.process.kill.callCount.should.equal(1);
			siriusd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			siriusd.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var siriusd = new siriusService(baseConfig);
			siriusd.shutdownTimeout = 300;
			siriusd.spawn = {};
			siriusd.spawn.process = new EventEmitter();
			siriusd.spawn.process.kill = sinon.stub();
			siriusd.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				siriusd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var siriusd = new siriusService(baseConfig);
			sinon.spy(siriusd, '_normalizeAddressArg');
			sinon.spy(siriusd, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			siriusd.nodes.push({
			...
			siriusd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				siriusd._getBalanceFromMempool.callCount.should.equal(1);
				siriusd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			siriusd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			siriusd.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			siriusd.getJsonBlock(0, function(err) {
				should.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			siriusd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			siriusd.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				siriusd._tryAllClients.callCount.should.equal(2);
				siriusd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			siriusd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
				siriusd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var siriusd = new siriusService(baseConfig);
			var emitter = new EventEmitter();
			siriusd.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(siriusd.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var siriusd = new siriusService(baseConfig);
			var emitter = new EventEmitter();
			siriusd.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			siriusd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			siriusd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			siriusd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			siriusd.unsubscribeBalance(emitter1);
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			siriusd.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			siriusd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var siriusd = new siriusService(baseConfig);
			...
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			siriusd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			siriusd.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var siriusd = new siriusService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			siriusd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(siriusd, 'unsubscribeBalanceAll');
			siriusd.unsubscribeBalance(emitter1);
			siriusd.unsubscribeBalanceAll.callCount.should.equal(1);
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			siriusd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			siriusd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			siriusd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			siriusd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			siriusd.unsubscribeBalanceAll(emitter1);
			siriusd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			siriusd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			siriusd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(siriusd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var siriusd = new siriusService(baseConfig);
			sinon.spy(siriusd, '_notifyBalanceSubscriber');
			siriusd._getAddressesFromTransaction = sinon.stub().returns(addresses);
			siriusd.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('siriusd/addressbalance', function(data) {
				...
				siriusd._notifyBalanceSubscriber.callCount.should.equal(1);
				siriusd._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			siriusd._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('siriusd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				siriusd.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			siriusd._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			siriusd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getDgpInfo(function(err, result) {
				should.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getMiningInfo(function(err, result) {
				should.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			siriusd.getStakingInfo(function(err, result) {
				should.exist(err);
				siriusd._tryAllClients.callCount.should.equal(1);
