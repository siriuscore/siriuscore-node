...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/siriuscore-node.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update siriuscore-node.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          'siriuscore-lib': '^v0.13.7',
          'siriuscore-node': '^v0.2.0'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/siriuscore-node.json');
