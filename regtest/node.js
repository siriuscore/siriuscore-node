...
describe('Node Functionality', function() {
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.siriusd.height === 150) {
            node.services.siriusd.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.siriusd.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('siriusd/hashblock', function(data) {
        bus.unsubscribe('siriusd/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.siriusd.height;
      node.services.siriusd.on('tip', function(height) {
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        ...
        node.services.siriusd.sendTransaction(tx.serialize(), function(err, hash) {
