...
var siriusRPC = require('siriusd-rpc');
...
/**
 * Provides a friendly event driven API to siriusd in Node.js. Manages starting and
 * stopping siriusd as a child process for application support, as well as connecting
 * to multiple siriusd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function sirius(options) {
  if (!(this instanceof sirius)) {
    return new sirius(options);
  }
...
util.inherits(sirius, Service);

sirius.dependencies = [];

sirius.DEFAULT_MAX_TXIDS = 1000;
sirius.DEFAULT_MAX_HISTORY = 50;
sirius.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
sirius.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
sirius.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
sirius.DEFAULT_SPAWN_RESTART_TIME = 5000;
sirius.DEFAULT_SPAWN_STOP_TIME = 10000;
sirius.DEFAULT_TRY_ALL_INTERVAL = 1000;
sirius.DEFAULT_REINDEX_INTERVAL = 10000;
sirius.DEFAULT_START_RETRY_INTERVAL = 5000;
sirius.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
sirius.DEFAULT_TRANSACTION_CONCURRENCY = 5;
sirius.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
sirius.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || sirius.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || sirius.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || sirius.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || sirius.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || sirius.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || sirius.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || sirius.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || sirius.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || sirius.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || sirius.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

sirius.prototype._initCaches = function() {
...
sirius.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
sirius.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
sirius.prototype.getPublishEvents = function() {
  return [
    {
...
sirius.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'siriusd/' + name, 'total:', this.subscriptions[name].length);
};

sirius.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'siriusd/' + name, 'total:', this.subscriptions[name].length);
};


sirius.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
sirius.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'siriusd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

sirius.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'siriusd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
sirius.prototype.unsubscribeAddressAll = function(emitter) {
...
sirius.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = sirius.DEFAULT_CONFIG_SETTINGS;
...
sirius.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
sirius.prototype._expandRelativeDatadir = function() {
...
sirius.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in siriusd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in siriusd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in siriusd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './sirius.conf');

  log.info('Using sirius config file:', configPath);
...
sirius.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of siriuscore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of siriuscore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of siriuscore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to siriusd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from siriusd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from siriusd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that siriusd is undergoing a reindex. ' +
             ...
             'of siriuscore-node services will start.');
    node._reindex = true;
  }
};
...
sirius.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
sirius.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

sirius.prototype._initChain = function(callback) {
  var self = this;
...
sirius.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
sirius.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/sirius.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/sirius.conf';
    }
  }
  return networkPath;
};

sirius.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
sirius.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('siriusd/hashblock', message.toString('hex'));
    }
  }

};

sirius.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
sirius.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('sirius Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
sirius.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
sirius.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
sirius.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
sirius.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('siriusd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
sirius.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('siriusd/rawtransaction', message.toString('hex'));
    }
...
sirius.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || sirius.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

sirius.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
sirius.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
sirius.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('sirius Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || sirius.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

sirius.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
sirius.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/siriusd.pid';
...
sirius.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('sirius process unexpectedly exited with code:', code);
        log.warn('Restarting sirius child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('sirius process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn siriusd.'));
      }
...
sirius.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to siriusd.'));
    }
 ...
 */
sirius.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('sirius configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
sirius.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
sirius.prototype.syncPercentage = function(callback) {
  var self = this;
...
sirius.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, siriuscore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
sirius.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
sirius.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
sirius.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
sirius.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in siriuscore is most recent to less recent
 ...
 * @param {Function} callback
 */
sirius.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
sirius.prototype._getConfirmationsDetail = function(transaction) {
...
sirius.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
sirius.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
sirius.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A siriuscoin transaction id
 * @param {Function} callback
 */
sirius.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
sirius.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
sirius.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
sirius.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
sirius.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
sirius.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
sirius.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
sirius.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
sirius.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
sirius.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a siriuscore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
sirius.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.getInfo = function(callback) {
...
sirius.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('siriusd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('siriusd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
sirius.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
sirius.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
sirius.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
sirius.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
sirius.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
sirius.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
sirius.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
sirius.prototype.getStakingInfo = function(callback) {
    var self = this,
