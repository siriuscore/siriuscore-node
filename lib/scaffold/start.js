...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for siriusd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and siriuscore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed siriuscore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './siriuscore-node.json');
  ...
  node.on('ready', function() {
    log.info('siriuscore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in siriuscore-node services directory
    ...
    if (servicePackage.siriuscoreNode) {
      serviceModule = service.name + '/' + servicePackage.siriuscoreNode;
    }
