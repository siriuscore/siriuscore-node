...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    // 'siriuscore-lib': 'https://github.com/siriusproject/siriuscore-lib.git#master',
    // 'siriuscore-node': 'https://github.com/siriusproject/siriuscore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/siriuscore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the siriuscore-node directory and configuration
